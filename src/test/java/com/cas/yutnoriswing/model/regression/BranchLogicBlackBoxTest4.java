...
@DisplayName("윷놀이 분기 로직 Black Box 테스트 - 4각형")
class BranchLogicBlackBoxTest4 {
    ...
    @Test
    @DisplayName("4각형: Edge0-5 분기점을 끼고 3칸 이동")
    ...
    @Test
    @DisplayName("4각형: Edge1-5 분기점을 끼고 3칸 이동")
    ...
    @Test
    @DisplayName("4각형: Edge2-5 분기점을 끼고 3칸 이동")
    ...
    @Test
    @DisplayName("4각형: 센터에서 나가는 분기 (ToCenter1 → ToCenter3)")
    void testCenterPath_ToCenter1() {
        ...
        // Then: ToCenter3-1에 도착해야 함 (직진 경로)
        BoardNode finalPos = tokenManager.getTokenPosition(token);
        assertEquals("ToCenter3-1", finalPos.getName());
    }

    @Test
    @DisplayName("4각형: 센터에서 나가는 분기 (ToCenter2 → ToCenter0)")
    void testCenterPath_ToCenter2() {
        ...
        // Then: ToCenter0-1에 도착해야 함 (직진 경로)
        BoardNode finalPos = tokenManager.getTokenPosition(token);
        assertEquals("ToCenter0-1", finalPos.getName());
    }    
