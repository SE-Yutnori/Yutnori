...
class InGameViewTest {
    ...
    void testValidateReorderInput_WhitespaceHandling() {
        List<Integer> originalResults = Arrays.asList(1, 2, 3);
        
        // 공백이 포함된 입력
        InGameView.ReorderResult result1 = InGameView.validateReorderInput(" 2, 1 , 3 ", originalResults);
        assertTrue(result1.isSuccess());
        assertEquals(Arrays.asList(2, 1, 3), result1.getReorderedResults());
        
        // 앞뒤 공백
        InGameView.ReorderResult result2 = InGameView.validateReorderInput("  3,2,1  ", originalResults);
        assertTrue(result2.isSuccess());
        assertEquals(Arrays.asList(3, 2, 1), result2.getReorderedResults());
    }

    @Test
    @DisplayName("순서 재배열 검증 - 중복값 처리")
    void testValidateReorderInput_DuplicateHandling() {
        List<Integer> originalResults = Arrays.asList(2, 2, 3); // 원본에 중복값 있음
        
        // 올바른 중복값 재배열
        InGameView.ReorderResult result1 = InGameView.validateReorderInput("2,3,2", originalResults);
        assertTrue(result1.isSuccess());
        assertEquals(Arrays.asList(2, 3, 2), result1.getReorderedResults());
        
        // 잘못된 중복값 (개수가 다름)
        InGameView.ReorderResult result2 = InGameView.validateReorderInput("2,2,2", originalResults);
        assertFalse(result2.isSuccess());
    }
} 